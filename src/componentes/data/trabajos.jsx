const trabajos = [
	{
		id: 1,
		categoria: 'diseño-web',
		thumb: {
			url: './assets/trabajos/landing.png',
			alt: 'Trabajo 1',
		},
		info: {
			nombre: 'Landing Page Basica',
			categoria: 'Diseño Web',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	{
		id: 2,
		categoria: 'desarrollo-web',
		thumb: {
			url: './assets/trabajos/linktree.png',
			alt: 'Trabajo 2',
		},
		info: {
			nombre: 'Linktree Personal',
			categoria: 'Desarrollo Web',
			contenido: (
				<>
					<p>
						Página personal estilo Linktree creada con HTML y CSS puro. Permite centralizar enlaces clave como CV, redes profesionales (LinkedIn, GitHub) y portafolio. El diseño es completamente responsivo, adaptado para dispositivos móviles y de escritorio. Se priorizó una estructura limpia, navegación simple y accesibilidad para usuarios.
					</p>

					<img loading="lazy" src="./assets/trabajos/imgLinktree.png" alt="" />
				</>
			),
		},
	},
	{
		id: 4,
		categoria: 'desarrollo-software',
		thumb: {
			url: './assets/trabajos/sistema.png',
			alt: 'Trabajo 4',
		},
		info: {
			nombre: 'Sistema Registro de Notas',
			categoria: 'Desarrollo de Software',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	{
		id: 5,
		categoria: 'diseño-web',
		thumb: {
			url: './assets/trabajos/portafolio.png',
			alt: 'Trabajo 5',
		},
		info: {
			nombre: 'Portafolio Personal',
			categoria: 'Diseño Web',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/Portafolio Light.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/Portafolio Dark.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	{
		id: 6,
		categoria: 'desarrollo-web',
		thumb: {
			url: './assets/trabajos/paginapublicitaria.png',
			alt: 'Trabajo 6',
		},
		info: {
			nombre: 'Pagina Publicitaria',
			categoria: 'Desarrollo Web',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	// {
	// 	id: 8,
	// 	categoria: 'desarrollo-software',
	// 	thumb: {
	// 		url: './assets/trabajos/enProceso.png',
	// 		alt: 'Trabajo 8',
	// 	},
	// 	info: {
	// 		nombre: '******',
	// 		categoria: 'Desarrollo de Software',
	// 		contenido: (
	// 			<>
	// 				<p>
	// 					Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
	// 					Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
	// 					officiis provident?
	// 				</p>

	// 				<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

	// 				<p>
	// 					Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
	// 					Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
	// 					officiis provident?
	// 				</p>

	// 				<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

	// 				<p>
	// 					Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
	// 					Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
	// 					officiis provident?
	// 				</p>
	// 			</>
	// 		),
	// 	},
	// },
	{
		id: 9,
		categoria: 'diseño-web',
		thumb: {
			url: './assets/trabajos/soniem.png',
			alt: 'Trabajo 9',
		},
		info: {
			nombre: 'Soniem CIMP',
			categoria: 'Diseño Web',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	{
		id: 10,
		categoria: 'desarrollo-web',
		thumb: {
			url: './assets/trabajos/rastaurant.png',
			alt: 'Trabajo 10',
		},
		info: {
			nombre: 'Restaurant',
			categoria: 'Desarrollo Web',
			contenido: (
				<>
					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/2.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>

					<img loading="lazy" src="./assets/trabajos/3.png" alt="" />

					<p>
						Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
						Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
						officiis provident?
					</p>
				</>
			),
		},
	},
	// {
	// 	id: 12,
	// 	categoria: 'desarrollo-software',
	// 	thumb: {
	// 		url: './assets/trabajos/enProceso.png',
	// 		alt: 'Trabajo 12',
	// 	},
	// 	info: {
	// 		nombre: '******',
	// 		categoria: 'Desarrollo de Software',
	// 		contenido: (
	// 			<>
	// 				<p>
	// 					Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quas iste doloribus saepe voluptatem.
	// 					Nihil quo beatae aut. Qui nam enim blanditiis obcaecati natus aliquid, rem facilis amet dolore
	// 					officiis provident?
	// 				</p>
	// 			</>
	// 		),
	// 	},
	// },
];

export default trabajos;
